...
class MockLocalPreferencesHandlerService {
  ...
  Future<bool> checkUserHistoryIntegrity() async {
    ...
    if (jsonUserHistory != null) {
      var result = _tryDecode(_cryptoTools.decrypt(kUserHistoryKeyEncrypted));
      if (!result) {
  ...
  changeData() async {
    var data = _instance.getString(kViwedTvShowAndMoviesUserIdKeyEncrypted);
    var data1 = _instance.getString(kFavoritesTvShowAndMoviesKeyEncrypted);
    var data2 = _instance.getString(kUserHistoryKeyEncrypted);
    print("$data / $data2 / $data2");
    await _instance.setString(
        kViwedTvShowAndMoviesUserIdKeyEncrypted, "${data!.toLowerCase()}");
    await _instance.setString(
        kFavoritesTvShowAndMoviesKeyEncrypted, "${data1!}ax");
    await _instance.setString(
        kUserHistoryKeyEncrypted, "${data2!.replaceAll("A", "B")}");
  }
}
